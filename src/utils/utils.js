...
const Utils = {
  /**
   * 判断是否选择日期 且 日期不为空，返回日期字符串
   * @param date moment格式的数组
   * @param index 数组下标
   * @param dateFormat 格式化时间格式，String
   */
  getStringFromMonent: (date, index, dateFormat) => {
    return date && date[index] && moment(date[index].valueOf()).format(dateFormat)
  },
  /**
   * 图片上传组件 变化处理，返回处理的fileList数组
   * @param file
   * @param fileList
   */
  uploadHandleChange: ({file, fileList}) => {
    const newFileList = fileList.filter(item => item.status !== "error")
      .map((item) => {
        if (item.response && item.response.errorCode === 0) {
          item.url = item.response.body;
          item.link = item.response.body;
        }
        return item;
      });
  ...
  dialog: {
    success: (msg, callback) => {
      message.success(msg, 3, callback ? callback : null);
    },
    error: (msg, callback) => {
      message.error(msg, 3, callback ? callback : null);
    },
    warning: (msg, callback) => {
      message.warning(msg, 3, callback ? callback : null);
    },
    loading: (msg, callback) => {
      message.loading(msg, 3, callback ? callback : null);
    }
  },
  imgResize:{
    param106() {
      // 小图尺寸
      return '?x-oss-process=image/resize,m_pad,h_60,w_100,color_FFFFFF'
    },
    param106fill() {
      // 小图尺寸
      return '?x-oss-process=image/resize,m_fill,h_60,w_100,color_FFFFFF'
    },
    param55() {
  ...
  formatDate: function (date, formatStr) {
    ...
    var arrWeek = ['日', '一', '二', '三', '四', '五', '六'],
      str = formatStr
        ...
        .replace(/MM/, $addZero(date.getMonth() + 1, 2))
        .replace(/M/g, date.getMonth() + 1)
        .replace(/dd|DD/, $addZero(date.getDate(), 2))
        .replace(/d|D/g, date.getDate())
        .replace(/hh|HH/, $addZero(date.getHours(), 2))
        .replace(/h|H/g, date.getHours())
        .replace(/mm|ii|II/, $addZero(date.getMinutes(), 2))
        .replace(/m|i|I/g, date.getMinutes())
